...
df_zhenbao_info$firstHeading[str_detect(df_zhenbao_info$firstHeading, "凤")]
```
...
df_zhenbao_user_result_property <- df_user_property_top2 %>% 
  select(firstHeading,property_type_1st,property_type_2nd) %>% 
  left_join(df_zhenbao_property_top2 %>% filter(cat == "天级珍宝") %>% mutate(zhenbao_tian = zhenbao) %>% select(-zhenbao,-cat), by = c("property_type_1st" = "property_type_1st", "property_type_2nd" = "property_type_2nd" )) %>% 
  group_by(firstHeading) %>% 
...
df_zhenbao_user_result <- df_zhenbao_user_result%>% 
  ...
  ## 根据已有珍宝判断现在最佳选择
  mutate(zhenbao_current_final = ifelse(zhenbao_tag == '已有' & !is.na(zhenbao_tag), zhenbao, 
                                        ifelse(coalesce(zhenbao_tag, zhenbao_tian_tag) == '已有' , zhenbao_tian,
                                               ifelse(coalesce(zhenbao_tag, zhenbao_tian_tag, zhenbao_hou_tag) == '已有' , zhenbao_hou,
                                                      ifelse(coalesce(zhenbao_tag, zhenbao_tian_tag, zhenbao_hou_tag, zhenbao_tian_single_tag) == '已有' , zhenbao_tian_single, 
                                                             NA))))) %>% 
  ## 判断现在是否最佳搭配
  left_join(owned_user_zhenbao_pair %>% mutate(pair_tag = '已有'), by = c("firstHeading" = "user", "zhenbao_current_final" = "zhenbao"))

df_zhenbao_user_result %>%
  filter(user_tag == "已有" & is.na(pair_tag)) %>%
  select(firstHeading,user_cat,zhenbao_final,zhenbao_current_final,pair_tag)
```
