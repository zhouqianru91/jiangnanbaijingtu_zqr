...
df_owned_zhenbao <- data.frame(zhenbao = owned_zhenbao_list)
...
# df_zhenbao_user_result_property <- 
  ...
  select(-property_type_1st,-property_type_2nd)
df_zhenbao_user_result_property
```
...
df_zhenbao_user_result <- df_user_info %>% 
  ...
  left_join(df_zhenbao_info %>% mutate(zhenbao = firstHeading) %>% select(-firstHeading,-zhenbao_desc), by = c("firstHeading" = "user_name")) %>% 
  left_join(df_zhenbao_user_result_property) %>% 
  mutate(zhenbao_final = coalesce(zhenbao,zhenbao_tian,zhenbao_hou)) %>% 
  select(firstHeading,user_cat,zhenbao, zhenbao_tian,zhenbao_hou,zhenbao_final) %>% 
  left_join(df_owned_user %>% mutate(user_tag = "已有"),by = c("firstHeading" = "user")) %>%
  # filter(!is.na(user_tag)) %>%
  left_join(df_owned_zhenbao %>% mutate(zhenbao_tag = "已有"), by = c("zhenbao_final" = "zhenbao")) %>% 
 #  # filter(!is.na(zhenbao_tag))
  left_join(owned_user_zhenbao_pair %>% mutate(pair_tag = '已有'), by = c("firstHeading" = "user", "zhenbao_final" = "zhenbao"))

df_zhenbao_user_result %>% 
  filter(user_tag == "已有" & zhenbao_tag == "已有")
```
