...
## 用户和属性匹配
## 天级 2个属性都匹配
df_zhenbao_user_result_property <- df_user_property_top2 %>% 
  select(firstHeading,property_type_1st,property_type_2nd) %>% 
  left_join(df_zhenbao_property_top2 %>% filter(cat == "天级珍宝" | cat == "专属珍宝") %>% mutate(zhenbao_tian = zhenbao) %>% select(-zhenbao,-cat), by = c("property_type_1st" = "property_type_1st", "property_type_2nd" = "property_type_2nd" )) %>% 
  group_by(firstHeading) %>% 
  mutate(rn = row_number(desc(property_value_1st))) %>% 
  filter(rn == 1 | is.na(rn)) %>% 
  select(-rn, -property_value_1st, -property_value_2nd)
df_zhenbao_user_result_property

## 候级 2个属性都匹配
df_zhenbao_user_result_property <- df_zhenbao_user_result_property %>% 
  left_join(df_zhenbao_property_top2 %>% filter(cat == "侯级珍宝") %>% mutate(zhenbao_hou = zhenbao ) %>% select(-zhenbao,-cat), by = c("property_type_1st" = "property_type_1st", "property_type_2nd" = "property_type_2nd" )) %>% 
  group_by(firstHeading) %>% 
  mutate(rn = row_number(desc(property_value_1st))) %>% 
  filter(rn == 1 | is.na(rn)) %>% 
  select(-rn, -property_value_1st, -property_value_2nd)
...
## 天级 1个属性匹配
df_zhenbao_user_result_property <- df_zhenbao_user_result_property %>% 
  left_join(df_zhenbao_property_top2 %>% filter(cat == "天级珍宝") %>% mutate(zhenbao_tian_single = zhenbao ) %>% select(-zhenbao,-cat,-property_type_2nd), by = c("property_type_1st" = "property_type_1st" )) %>% 
  group_by(firstHeading) %>% 
  mutate(rn = row_number(desc(property_value_1st))) %>% 
  filter(rn == 1 | is.na(rn)) %>% 
  select(-rn, -property_value_1st, -property_value_2nd)
...
df_zhenbao_user_result <- df_user_info %>% 
  select(firstHeading,user_cat) %>% 
  left_join(df_zhenbao_info %>% 
              mutate(zhenbao = firstHeading) %>% 
              select(-firstHeading,-zhenbao_desc), by = c("firstHeading" = "user_name")) %>% 
  left_join(df_zhenbao_user_result_property) %>% 
  # mutate(zhenbao_final = coalesce(zhenbao,zhenbao_tian,zhenbao_hou,zhenbao_tian_single)) %>% 
  select(firstHeading,user_cat,zhenbao, zhenbao_tian,zhenbao_hou,zhenbao_tian_single)  #, zhenbao_final

## 验证是否拥有
df_zhenbao_user_result <- df_zhenbao_user_result %>% 
  ## 判断用户是否已有
  left_join(df_owned_user %>% mutate(user_tag = "已有"),by = c("firstHeading" = "user")) %>%
  ## 判断专属珍宝是否已有
  left_join(df_owned_zhenbao %>% mutate(zhenbao_tag = "已有"), by = c("zhenbao" = "zhenbao")) %>% 
  ## 判断天级珍宝是否已有
  left_join(df_owned_zhenbao %>% mutate(zhenbao_tian_tag = "已有"), by = c("zhenbao_tian" = "zhenbao")) %>% 
  ## 判断侯级珍宝是否已有
  left_join(df_owned_zhenbao %>% mutate(zhenbao_hou_tag = "已有"), by = c("zhenbao_hou" = "zhenbao")) %>%
  ## 判断仅符合一个属性的天级珍宝是否已有
  left_join(df_owned_zhenbao %>% mutate(zhenbao_tian_single_tag = "已有"), by = c("zhenbao_tian_single" = "zhenbao")) #%>%
  ## 判断理论最佳选择是否已有
  # left_join(df_owned_zhenbao %>% mutate(zhenbao_final_tag = "已有"), by = c("zhenbao_final" = "zhenbao")) 

## current最佳珍宝
df_zhenbao_user_result <- df_zhenbao_user_result %>%
  ## 根据已有珍宝判断现在最佳选择
  mutate(zhenbao_current_best = ifelse(zhenbao_tag == '已有' & !is.na(zhenbao_tag), zhenbao, 
                                        ifelse(coalesce(zhenbao_tag, zhenbao_tian_tag) == '已有' & !is.na(coalesce(zhenbao_tag, zhenbao_tian_tag)) , zhenbao_tian,
                                               ifelse(coalesce(zhenbao_tag, zhenbao_tian_tag, zhenbao_hou_tag) == '已有' & !is.na(coalesce(zhenbao_tag, zhenbao_tian_tag,zhenbao_hou_tag)) , zhenbao_hou,
                                                      ifelse(coalesce(zhenbao_tag, zhenbao_tian_tag, zhenbao_hou_tag, zhenbao_tian_single_tag) == '已有' & !is.na(coalesce(zhenbao_tag, zhenbao_tian_tag,zhenbao_hou_tag,zhenbao_tian_single_tag))  , zhenbao_tian_single, 
                                                             NA))))) %>%
  ## 现在用的
  left_join(owned_user_zhenbao_pair %>% mutate(current_zhenbao = zhenbao) %>% select(-zhenbao), by = c("firstHeading" = "user")) %>% 
  ## 判断现在是否最佳搭配
  left_join(owned_user_zhenbao_pair %>% mutate(pair_tag = '已有'), by = c("firstHeading" = "user", "zhenbao_current_best" = "zhenbao")) %>% 
  select(firstHeading,user_cat,user_tag,current_zhenbao,zhenbao_current_best,pair_tag,zhenbao,zhenbao_tag,zhenbao_tian,zhenbao_tian_tag,zhenbao_hou,zhenbao_hou_tag,zhenbao_tian_single,zhenbao_tian_single_tag,)
```


```{r}
df_zhenbao_user_result %>%
  filter(user_tag == "已有" &!is.na(zhenbao_current_best)) # %>% # is.na(pair_tag) &
  # select(firstHeading,user_cat,zhenbao_final,zhenbao_current_best,pair_tag) 
```
```{r}
user_current = '范泊宁'
df_zhenbao_user_result %>%
...
```{r}
# zhenbao_current == '打神鞭'
# zhenbao_current <- owned_user_zhenbao_pair$zhenbao[owned_user_zhenbao_pair$user == user_current]
df_zhenbao_property_top2 %>% 
  # filter(zhenbao == zhenbao_current)
  filter(zhenbao %in% c('八景宫灯','红鸾铜镜'))
```
